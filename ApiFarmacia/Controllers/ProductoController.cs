...
public class ProductoController : BaseApiController
{
    ...
    [HttpGet("mediExpiraAntes")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]

    public async Task<ActionResult<IEnumerable<ProductoDto>>> GetExpire()
    {
        var producto = await unitOfWork.Productos.GetMediExpireBeforeDate();
        if (producto == null)
        {
            return BadRequest("No se encontraron productos a expirar el 1 de Enero del 2024");
        }
        return mapper.Map<List<ProductoDto>>(producto);
    }
